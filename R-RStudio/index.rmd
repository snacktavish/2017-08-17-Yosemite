...
I've modified them in part with my own text and in part with text from Software Carpentry and Data Carpentry, specifically: 

- [R for reproducible scientific analysis/Seeking help](http://swcarpentry.github.io/r-novice-gapminder/03-seeking-help.html)
...
Another was is to try out RStudio's completion facility: type the first few characters, press TAB, add characters until you disambiguate, then press return.
```{r}
this_is_a_really_long_name
```
...
# functionName(argument1 = my_first_argument, argument2 = my_second_argument...)
```
...
# Starting an analytical workflow

We're going to do a few firsts here: we're going to 

- create an RStudio project (.Rproj) 
- create our first R script
- explore our first dataframe
...
We will use the `download.file()` function to download data from figshare online. Once you run this, you should see the file appear in the data folder.
...
A few things to note with the above: the function call is written across 2 lines, and I've even added spaces so that the `=` line up and it's a bit easier to read. R is OK with extra white space, so we can use some whitespace to make it easier to read.

## Explore the data

Let's read the data into R and save it to a variable called `surveys`. 
